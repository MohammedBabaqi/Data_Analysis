...
@st.cache_data
def load_data():
    file_path = r"c:\Users\mbaba\Desktop\Data Analysis\Orders.csv"
    # Skip the first 4 lines based on initial exploration
    df = pd.read_csv(file_path, skiprows=4)
    # Drop empty rows
    df = df.dropna(how='all')
    # Filter out any header repetition or clearly invalid rows if they exist
...
st.dataframe(filtered_df.drop(columns=['Lat', 'Lng', 'Email', 'Phone Number']).head(100), use_container_width=True)

st.markdown("---")
st.markdown("Dashboard enhanced by Antigravity AI")
